def question_j
  ...
  puts "Bienvenue a toi."
  puts "Nous avons une liste de journaliste et plusieurs questions sont poser. "
  choix = ""
  puts
  ...
  while choix != "q" && choix != "Q"
    print "Tape le numero correspondant à la question pour voir la reponse (menu pour revoir les choix) : -> "
    choix = gets.chomp
    puts
	  if choix == "1"
		  puts "1- Combien y a-t-il de handle dans cette array ?"
  		...
  		puts "6- Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
	  	puts journalist.sort_by { |word| word.length }
		  puts
    elsif choix == "7"
  		puts "7- Quelle est la position dans l'array de la personne @epenser ?"
	  	puts journalist.find_index("@epenser")
		  puts
    elsif choix == "8"
  		puts "8- Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)"
	  	puts
      ...
      puts "8- Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)"
      puts
    elsif choix == "q" || choix == "Q"
      puts "Merci et à bientot !"
      puts
    else
      puts ">>>>> Entré Invalide veuillez recommencer en faisant attention. Merci. <<<<<<"
      puts
    end
  end
end
